...
arma::mat spinw::arma_spinwave(double* qRange, spinwave_opt options) {
    ...
    arma::mat k(&mag_str1.k[0], 3, mag_str1.nMagExt, false, true);
    if (options.tid == -2) {
        std::cout << "k" << std::endl;
        std::cout << k << std::endl;
    }
    ...
    if (options.tid == -2) {
        ...
        std::cout << arma::mat(&(unit1.qmat[0]), 3, 3, false, true) << std::endl;
    }

    hkl = arma::mat(&(unit1.qmat[0]), 3, 3, false, true) * hkl;
    if (options.tid == -2) {
...
//    }
    ...
    arma::cube rotc(&(twin1.rotc[0]), 3, 3, twin1.nTwin, false, true);
